...
char *display_ballot_tracker(struct ballot_tracker tracker) {
    ...
    // BUT take care: if there is no final space to begin with...
    if(len <= 1) len = 1;
    ...
    // We let it slide if we come up one byte short when the original plan was
    // to produce an empty string anyway.
    assert(cur_idx == len || (cur_idx == 0 && len == 1));
    // Null terminator.
    result[len-1] = '\0';
